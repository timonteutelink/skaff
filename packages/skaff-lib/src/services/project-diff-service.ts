...
async function recursivelyAddAutoInstantiatedTemplatesToProjectSettings(
  ...
  for (const autoInstantiatedTemplate of templatesToAutoInstantiate || []) {
    ...
    projectSettings.instantiatedTemplates.push({
      ...
      templateCommitHash: subTemplate.commitHash ?? currentTemplateToAddChildren.commitHash,
      templateRepoUrl: subTemplate.repoUrl ?? currentTemplateToAddChildren.repoUrl,
      templateBranch: subTemplate.branch ?? currentTemplateToAddChildren.branch,
...
export async function generateModifyTemplateDiff(
  ...
  const targetTemplate = template.data.findSubTemplate(
