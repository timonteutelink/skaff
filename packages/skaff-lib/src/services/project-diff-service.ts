...
import {
  ...
  latestMigrationUuid,
} from "./template-migration-service";
...
async function recursivelyModifyAutoInstantiatedTemplatesInProjectSettings(
  ...
  projectSettings: ProjectSettings,
  currentTemplateToAddChildren: Template,
  currentTemplateToAddChildrenInstanceId: string,//TODO NOW
  currentTemplateToAddChildrenFinalSettings: FinalTemplateSettings, // TODO Force autoInstantiatedTemplate to be direct child of template. If nested use 'children' to autoinstantiate parent and child. Then also merge this modify function with the add function below if possible
): Promise<Result<ProjectSettings>> {
  for (const autoInstantiatedTemplate of templatesToAutoInstantiate || []) {
    const existingTemplateIndex =
      projectSettings.instantiatedTemplates.findIndex(
        (template) =>
          ...
          template.parentId === currentTemplateToAddChildrenInstanceId &&
    ...
    const autoGeneratedTemplateUserSettings = anyOrCallbackToAny(
      ...
      currentTemplateToAddChildrenFinalSettings,
    );
    ...
    const newFullTemplateSettings = Project.getFinalTemplateSettings(
      ...
      currentTemplateToAddChildrenInstanceId,
    );
...
async function recursivelyAddAutoInstantiatedTemplatesToProjectSettings(
  ...
  for (const autoInstantiatedTemplate of templatesToAutoInstantiate || []) {
    ...
    projectSettings.instantiatedTemplates.push({
      ...
      templateRepoUrl: currentTemplateToAddChildren.repoUrl,
      templateBranch: currentTemplateToAddChildren.branch,
  ...
  const newProjectSettings: ProjectSettings = {
    ...
    instantiatedTemplates: [
      ...destinationProject.instantiatedProjectSettings.instantiatedTemplates,
      {
        ...
        lastMigration: latestMigrationUuid(template.config.migrations),
      },
    ],
  };
...
export async function generateUpdateTemplateDiff(
  ...
  for (const instantiated of newProjectSettings.instantiatedTemplates) {
    ...
    const migrationResult = applyTemplateMigrations(
