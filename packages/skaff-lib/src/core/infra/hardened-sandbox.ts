 ...
 * - Date (non-deterministic, not needed by templates/plugins)
...
function createCompartmentGlobals(
  ...
  return {
    ...
    // SECURITY: Explicitly remove Date to prevent non-deterministic behavior
    Date: undefined,
  };
}
...
// =============================================================================
// SECURE INVOCATION HELPERS
// =============================================================================
// These functions provide type-safe, secure wrappers for invoking sandboxed
// functions from plugins and template configurations. They ensure:
// - All inputs are hardened (deeply frozen) before invocation
// - All outputs are hardened before being returned
// - The function signature is validated
...
/**
 * Result type for secure invocations that may fail.
 */
export type SecureInvocationResult<T> =
  | { success: true; data: T }
  | { success: false; error: string };

/**
 * Securely invokes a mapFinalSettings function from a template config.
 *
 * This function:
 * 1. Creates readonly views of all input data
 * 2. Hardens all inputs to prevent mutation
 * 3. Invokes the function in the sandbox
 * 4. Hardens and validates the output
 *
 * @param sandbox - The sandbox service to use
 * @param mapFn - The mapFinalSettings function from the template
 * @param input - The input containing project settings, template settings, and parent settings
 * @returns The computed final settings or an error
 */
export function secureInvokeMapFinalSettings<TInput, TOutput>(
  sandbox: HardenedSandboxService,
  mapFn: (input: TInput) => TOutput,
  input: TInput,
): SecureInvocationResult<TOutput> {
  try {
    const result = sandbox.invokeFunction(mapFn, input);
    return { success: true, data: result };
  } catch (error) {
    return {
      success: false,
      error: `mapFinalSettings execution failed: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

/**
 * Securely invokes a plugin's getFinalTemplateSettings function.
 *
 * This function ensures that plugin settings computation is:
 * - Deterministic (same inputs = same outputs)
 * - Sandboxed (no access to filesystem, network, etc.)
 * - Immutable (inputs and outputs are frozen)
 *
 * @param sandbox - The sandbox service to use
 * @param getFn - The plugin's getFinalTemplateSettings function
 * @param input - The plugin final settings input
 * @returns The computed plugin settings or an error
 */
export function secureInvokePluginGetFinalSettings<TOutput>(
  sandbox: HardenedSandboxService,
  getFn: (input: PluginFinalSettingsInput) => TOutput,
  input: PluginFinalSettingsInput,
): SecureInvocationResult<TOutput> {
  try {
    const result = sandbox.invokeFunction(getFn, input);
    return { success: true, data: result };
  } catch (error) {
    return {
      success: false,
      error: `Plugin getFinalTemplateSettings execution failed: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

/**
 * Securely invokes an AnyOrCallback function from a template config.
 *
 * Many template config properties can be either a value or a callback.
 * This function handles both cases securely:
 * - If value: returns it as-is (hardened)
 * - If callback: invokes it in the sandbox with hardened settings
 *
 * @param sandbox - The sandbox service to use
 * @param valueOrCallback - The value or callback to invoke
 * @param settings - The final settings to pass if it's a callback
 * @returns The resolved value or an error
 */
export function secureResolveAnyOrCallback<TSettings, TValue>(
  sandbox: HardenedSandboxService,
  valueOrCallback: TValue | ((settings: TSettings) => TValue),
  settings: TSettings,
): SecureInvocationResult<TValue> {
  try {
    if (typeof valueOrCallback === "function") {
      const result = sandbox.invokeFunction(
        valueOrCallback as (settings: TSettings) => TValue,
        settings,
      );
      return { success: true, data: result };
    }
    // For non-function values, just harden and return
    return { success: true, data: harden(valueOrCallback) as TValue };
  } catch (error) {
    return {
      success: false,
      error: `Callback execution failed: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

/**
 * Securely invokes a side effect transform function.
 *
 * Side effect transforms are pure functions that compute new file contents
 * from template settings and existing file contents. This function ensures
 * the transform is executed safely and deterministically.
 *
 * @param sandbox - The sandbox service to use
 * @param transform - The side effect transform function
 * @param input - The input containing template settings and existing contents
 * @returns The new file contents (or null to skip) or an error
 */
export function secureInvokeSideEffectTransform<TSettings>(
  sandbox: HardenedSandboxService,
  transform: (input: {
    templateSettings: TSettings;
    existingContents?: string;
  }) => string | null,
  input: { templateSettings: TSettings; existingContents?: string },
): SecureInvocationResult<string | null> {
  try {
    const result = sandbox.invokeFunction(transform, input);
    return { success: true, data: result };
  } catch (error) {
    return {
      success: false,
      error: `Side effect transform execution failed: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

/**
 * Securely invokes a Handlebars helper function.
 *
 * Handlebars helpers from templates are user-defined functions that must
 * be sandboxed. This wrapper ensures they cannot escape the sandbox or
 * cause non-deterministic behavior.
 *
 * @param sandbox - The sandbox service to use
 * @param helper - The helper function
 * @param args - The arguments to pass to the helper
 * @returns The helper result or throws if execution fails
 */
export function secureInvokeHandlebarsHelper(
  sandbox: HardenedSandboxService,
  ...
  try {
    return sandbox.invokeFunctionWithArgs(helper, ...args);
  } catch (error) {
    throw new Error(
      `Handlebars helper execution failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

/**
 * Creates a hardened copy of an object suitable for passing to sandboxed code.
 *
 * This is a convenience function that ensures the object is deeply frozen
 * and cannot be mutated by sandboxed code.
 *
 * @param obj - The object to harden
 * @returns A hardened (frozen) copy of the object
 */
export function hardenForSandbox<T>(obj: T): Readonly<T> {
  ensureHardenedEnvironment();
  return harden(obj) as Readonly<T>;
}
