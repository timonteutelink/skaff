...
Mounting your config directory keeps the web app aligned with the CLI. Open [http://localhost:3000](http://localhost:3000) once the container starts.
...
Use the CLI or the **Settings** page to add template repositories. Both surfaces update the same configuration file, so changes are shared instantly.
...
Return to the UI later to apply template updates, add subtemplates, or run template-defined commands without leaving the browser.
