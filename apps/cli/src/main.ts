...
program
	...
	.action(async () => {
		...
		const userTemplateSettings: UserTemplateSettings = JSON.parse(answers.settings);

		const template = await ROOT_TEMPLATE_REGISTRY.findTemplate(answers.templateName);
		if ('error' in template) {
			console.error(template.error);
			process.exit(1);
		}

		const result = await template.data.instantiateNewProject(userTemplateSettings, answers.parentDirPath, answers.projectName);
		if ('error' in result) {
...
program
	...
	.action(async () => {
		...
		const rootTemplate = await ROOT_TEMPLATE_REGISTRY.findTemplate(answers.rootTemplateName);
		...
		const project = await PROJECT_REGISTRY.findProject(answers.destinationProjectName);
		if (!project) {
			console.error('Destination project not found');
			process.exit(1);
		}

		const result = await subTemplate.templateInExistingProject(userTemplateSettings, project, answers.parentInstanceId);
		if ('error' in result) {
