...
import type { ParsedFile, DiffHunk } from "./parse-git-diff"
  ...
  return (
    <Card className="h-full">
      <CardHeader className="py-3 flex flex-row items-center justify-between">
        <CardTitle className="text-sm font-medium truncate">{file.path}</CardTitle>
      ...
      <CardContent className="p-0">
        <ScrollArea className="h-[calc(100vh-450px)]">
          {viewMode === "split" ? <SplitDiffView file={file} /> : <UnifiedDiffView file={file} />}
        </ScrollArea>
      </CardContent>
    </Card>
  )
...
function DiffHunkSplit({ hunk, side }: { hunk: DiffHunk; side: "old" | "new" }) {
  return (
    <div className="mb-4">
      ...
      {hunk.lines.map((line, lineIndex) => {
        ...
        if ((side === "old" && prefix === "+") || (side === "new" && prefix === "-")) {
          return null
        }
