...
export function Tree<T extends { id: string; children?: T[] }>({
	...
	return (
		<ArbTree<T> openByDefault={openByDefault} data={data} rowHeight={rowHeight} width={width}>
			{(props) => {
				const { node } = props;
				...
				return renderLabel
					? renderLabel(node.data, isSelected, node.toggle, node.isOpen, !!hasChildren, props.style, onClick)
					: defaultLabel;
			}}
		</ArbTree>
	);
