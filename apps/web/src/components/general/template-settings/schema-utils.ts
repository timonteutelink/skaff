...
export function buildSchemaAndDefaults(jsonSchema: any, path = ""): SchemaResult {
  ...
  if (jsonSchema.type === "object" && !jsonSchema.properties && jsonSchema.additionalProperties) {
  ...
  if (jsonSchema.type === "object" && jsonSchema.properties) {
    ...
    const propertiesToProcess = jsonSchema.pick
      ? Object.fromEntries(Object.entries(jsonSchema.properties).filter(([key]) => jsonSchema.pick.includes(key)))
      : // If omit is specified, filter properties to exclude those
      jsonSchema.omit
        ? Object.fromEntries(Object.entries(jsonSchema.properties).filter(([key]) => !jsonSchema.omit.includes(key)))
        : jsonSchema.properties

    Object.entries(propertiesToProcess).forEach(([key, value]: [string, any]) => {
      ...
      if (value.anyOf) {
        ...
        schemaShape[key] = isRequiredField ? unionSchema : unionSchema.optional()
        ...
        schemaShape[key] = isRequiredField ? recordSchema : recordSchema.optional()
        ...
        schemaShape[key] = isRequiredField ? tupleSchema : tupleSchema.optional()
        ...
        // Handle native enums
        const { schema: enumSchema, defaults: enumDefaults } = buildNativeEnumSchema(value)
        schemaShape[key] = isRequiredField ? enumSchema : enumSchema.optional()
...
export function buildNativeEnumSchema(jsonSchema: any): { schema: z.ZodType<any>; defaults: any, required: {} } {
  ...
  const schema = jsonSchema.default !== undefined ? enumSchema.default(jsonSchema.default) : enumSchema
...
export function buildDiscriminatedUnionSchema(jsonSchema: any, path = ""): SchemaResult {
    ...
    for (const [key, prop] of Object.entries(firstVariant)) {
      if ((prop as any).const !== undefined) {
        ...
        const allVariantsHaveDiscriminator = jsonSchema.anyOf.every(
          (variant: any) => variant.properties?.[key] && variant.properties[key].const !== undefined,
        )

        if (allVariantsHaveDiscriminator) {
          // Check that all discriminator values are unique
          const discriminatorValues = jsonSchema.anyOf.map((variant: any) => variant.properties[key].const)
  ...
  jsonSchema.anyOf.forEach((variant: any, index: number) => {
    ...
    if (discriminatorValue === undefined) {
      throw new Error(`Discriminator value for key '${discriminator}' is missing in variant ${index}`)
    }
  ...
  const unionSchema = z.discriminatedUnion(
    discriminator,
    variants as [z.ZodDiscriminatedUnionOption<string>, ...z.ZodDiscriminatedUnionOption<string>[]]
  )

  // Use the first variant's defaults as the initial defaults
  const firstVariantDiscriminator = jsonSchema.anyOf[0].properties[discriminator].const
...
export function buildTupleSchema(jsonSchema: any, path = ""): SchemaResult {
  ...
  jsonSchema.items.forEach((itemSchema: any, index: number) => {
    ...
    if (itemSchema.type === "object" && itemSchema.properties) {
      ...
      // Handle primitive types
      const { schema: fieldSchema, defaults: fieldDefault } = buildFieldSchema(itemSchema, true)
...
export function buildRecordSchema(jsonSchema: any, path = ""): SchemaResult {
  ...
  const { schema: valueSchema } = buildFieldSchema(jsonSchema.additionalProperties, false)
