...
export function renderInputByType({
  property,
  field,
  isReadOnly,
}: InputRendererProps) {
  ...
  if (
    ...
    (Array.isArray(property.type) &&
      (property.type.includes("number") || property.type.includes("integer")))
  ) {
      ...
      return (
        <Popover>
          ...
          <PopoverContent className="w-full p-0">
            <Command>
              <CommandInput
                placeholder={`Search ${property.title || field.name}...`}
              />
              <CommandList>
