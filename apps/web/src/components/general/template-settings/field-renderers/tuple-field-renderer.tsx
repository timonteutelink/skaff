          ...
          <FormControl>
            <Card className="overflow-visible shadow-sm">
              <CardHeader className="p-3 bg-muted/20">
                <div className="text-sm font-medium">Fixed-length array ({tupleItems.length} items)</div>
              </CardHeader>
                    ...
                    return (
                      <div key={index} className="border-b pb-4 last:border-0 last:pb-0">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 pl-4 border-l-2 border-muted">
                          {Object.entries(itemSchema.properties).map(([key, value]: [string, any]) => {
                            return renderFormField(key, value, itemPath, itemSchema.required || [])
                          })}
                        </div>
                      </div>
                    )
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            {itemSchema.description && (
                              <p className="text-xs text-muted-foreground mt-1">{itemSchema.description}</p>
                            )}
                            <FormMessage />
                          </FormItem>
                        )}
