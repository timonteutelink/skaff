  ...
  return (
    <Card key={item.id} className="overflow-visible">
      <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          <Button type="button" variant="ghost" size="sm" onClick={() => toggleItemExpansion(index)}>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="capitalize">
                        ...
                        {itemRequiredFields.includes(propKey) && <span className="text-destructive ml-1">*</span>}
                      </FormLabel>
                      <FormControl>{renderInputByType({ property: propValue, field, isReadOnly })}</FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex-1 space-y-0">
              <FormControl>{renderInputByType({ property: property.items, field, isReadOnly })}</FormControl>
              <FormMessage />
            </FormItem>
          )}
        />
